...
export class HaliteVisualizerControls {
    ...
    attach(el) {
        ...
        let infoPanel = $("<div>").addClass("halite-info-panel");
        infoPanel.appendTo(el);
        infoPanel.css("width", CONTAINER_SIZE - VISUALIZER_SIZE);
        ...
        this.visualizer.onUpdate = () => {
            ...
            let subframe = this.replay.frames[this.visualizer.frame][this.visualizer.substep];
            let count = {};
            for (let ship of subframe.ships) {
                let owner = ship.owner;
                if (typeof count[owner] === "undefined") {
                    count[owner] = 0;
                }
                count[owner]++;
            }

...
do
	...
	BOTSTARTCOMMANDS="$BOTSTARTCOMMANDS '/usr/bin/docker run --net=none --memory=\"256m\" --cpu-shares=1024 -i -v $PWD/$BOT:$PWD/$BOT virtual_machine sh -c \"cd $PWD/$BOT && ./$RUNFILE\"' '$BOTNAME'";
done

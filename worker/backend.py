...
def getTask():
    ...
    content = requests.get(
        MANAGER_URL + "task", params={"apiKey": API_KEY}).text
    if content == "null":
        return None
    else:
...
def getBotHash(userID):
    ...
    result = requests.get(
        MANAGER_URL + "botHash", params={"apiKey": API_KEY, "userID": userID})
    print(result.text)
...
def storeBotLocally(userID, storageDir):
    ...
    while iterations < 100:
        remoteZip = urllib.request.urlopen(
            MANAGER_URL + "botFile?apiKey=" + str(API_KEY) + "&userID=" + str(userID))
        zipFilename = remoteZip.headers.get(
...
def storeBotRemotely(userID, zipFilePath):
    ...
    while iterations < 100:
        r = requests.post(MANAGER_URL + "botFile", data={
                          "apiKey": API_KEY, "userID": str(userID)}, files={"bot.zip": zipContents})
...
def compileResult(userID, didCompile, language):
    ...
    r = requests.post(MANAGER_URL + "compile", data={
                      "apiKey": API_KEY, "userID": userID, "didCompile": int(didCompile), "language": language})


def gameResult(width, height, users, replayPath):
    """Posts the result of a game task"""
    r = requests.post(MANAGER_URL + "game", data={"apiKey": API_KEY, "mapWidth": str(width), "mapHeight": str(
        height), "users": json.dumps(users)}, files={os.path.basename(replayPath): open(replayPath, "rb").read()})

...
def check_path(path, errors):
    if not os.path.exists(path):
        errors.append("Output file " + str(
            os.path.basename(path)) + " was not created.")
        return False
    else:
        return True
...
comp_args = {
    # lang : ([list of compilation arguments], ...)
        ...
        # in order for the maximum heap size RTS flag to work on the
        # executable.
...
languages = (
    # Language(name, output file,
        ...
        Language("C++11", BOT, "MyBot.cpp",
                 ...
                 (["*.c", "*.cpp", "*.cc"],
                  TargetCompiler(
                  comp_args["C++11"][0], targets["C++"])),
                     (["*.o"], ExternalCompiler(comp_args["C++11"][1]))
                 ]
                 ),
        Language("Clojure", BOT + ".clj", "MyBot.clj",
                 "java -Xmx%sm -cp /usr/share/java/clojure.jar:. clojure.main MyBot.clj" % (
                     MEMORY_LIMIT,),
                 [],
        ...
        Language("Erlang", "my_bot.beam", "my_bot.erl",
                 "erl -hms" + \
                 str(MEMORY_LIMIT) + \
                 "m -smp disable -noshell -s my_bot start -s init stop",
                 ["*.beam"],
        ...
        Language("Groovy", BOT + ".jar", "MyBot.groovy",
                 "java -Xmx" + \
                 str(MEMORY_LIMIT) + \
                 "m -cp MyBot.jar:/usr/share/groovy/embeddable/groovy-all-1.7.5.jar MyBot",
                 ["*.class, *.jar"],
        ...
        Language("Scala", BOT + ".scala", "MyBot.scala",
                 'scala -J-Xmx' + \
                 str(MEMORY_LIMIT) + 'm -howtorun:object MyBot',
                 ["*.scala, *.jar"],
...
def detect_language(bot_dir):
    ...
    with CD(bot_dir):
        # Autodetects the language of the entry in the current working
        # directory
        detected_langs = [

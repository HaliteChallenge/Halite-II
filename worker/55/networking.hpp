...
static std::string serializeMoveSet(const std::map<hlt::Location, unsigned char> & moves) {
	std::ostringstream oss;
	for(auto a = moves.begin(); a != moves.end(); ++a) oss << a->first.x << " " << a->first.y << " " << (int)a->second << " ";
	return oss.str();
}
...
static void sendFrame(const std::map<hlt::Location, unsigned char> &moves) {

...
int main(int argc, char* args[])
{
	...
	for (unsigned int a = 0; a < rankings.size(); a++) victoryOut += "In place #" + std::to_string(a + 1) + " is player "  + std::to_string(rankings[a].first) + " named " + my_game->getName(rankings[a].first) + " with a score of " + std::to_string(rankings[a].second).substr(0, 5) + "\n";
	std::cout << victoryOut;

...
* Players have 3 ships at game start
* Players move ships around a board using three commands
...
* Ships may be destroyed through battle with each other or collision with planets
* Ships have an initial health and lose health as they battle
* If ships collide with each other or a planet they die

#### Planets are for producing new ships
* At the start of a game, all planets are neutral and players own none of them
* Only one team can dock on a planet at a time and can dock a set number of ships determined by the size of the planet
* When a ship docks on a planet, the planet produces ships for the docked team
* Planets produce at a constant rate per ship with no limit to ship production
* Planets can be destroyed through collisions with ships
* The number of planets is limited and varies by the size of the map
* The sizes and locations of planets also varies map to map

#### A player wins if either of the following conditions are met:
* They are the sole survivor
* They occupy all planets
* If time runs out and neither of these conditions are met, tiebreaker rules apply:

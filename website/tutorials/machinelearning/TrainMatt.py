...
def loadGame(filename):
    ...
    try:
        ...
        # Get production
        productions = [int.from_bytes(gameFile.read(1), byteorder='big')
        ...
        for frameIndex in range(numFrames - 1):
            # Frames
            frames.append(
                GameMap(width=width, height=height, numberOfPlayers=numPlayers))
            x = 0
            y = 0
            while y < height:
                ...
                for a in range(numTiles):
                    frames[-1].contents[y][x] = Site(ownerID, int.from_bytes(
                        gameFile.read(1), byteorder='big'), productions[y * width + x])
                    ...
                    if x == width:
                        ...
                        if y == height:
                            break
            # Moves
            moves.append({(index % width, math.floor(index / width)): int.from_bytes(gameFile.read(1), byteorder='big')
            ...
            for y in range(gameMap.height):
                for x in range(gameMap.width):
                    ...
                    if gameMap.getSite(loc).owner == mattID:
                        box = [gameMap.getSite(gameMap.getLocation(loc, NORTH), WEST), gameMap.getSite(loc, NORTH), gameMap.getSite(gameMap.getLocation(loc, NORTH), EAST), gameMap.getSite(
                            loc, EAST), gameMap.getSite(gameMap.getLocation(loc, SOUTH), EAST), gameMap.getSite(loc, SOUTH), gameMap.getSite(gameMap.getLocation(loc, SOUTH), WEST), gameMap.getSite(loc, WEST)]
                        nnInput = []
                        for site in box:
                            nnInput += [1 if site.owner == mattID else -1, float(
                                site.strength / 255), float(site.production / maxProduction)]
                        inputs.append(nnInput)
                        correctOutputs.append(
                            [1 if a == moves[turnIndex][(x, y)] else 0 for a in range(5)])
...
def trainModel():
    ...
    model.fit(trainingInputs, trainingOutputs,

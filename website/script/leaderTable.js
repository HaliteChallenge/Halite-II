var leaderTable = {
    ...
    render: function() {
        ...
        for(var a = 0; a < this.submissions.length; a++) {
            ...
            this.$tableBody.append("<tr id='user" + user.userID + "'><th scope='row'>"+(this.startingIndex+a)+"</th><td><a class='username' href='user.php?userID="+user.userID+"'>"+user.username+"</a></td><td><a href='leaderboard.php?field=language&value="+user.language+"&heading="+user.language+"'>"+user.language+"</a></td><td><a href='leaderboard.php?field=organization&value="+user.organization+"&heading="+user.organization+"'>"+user.organization+"</a></td><td>"+score+"</td></tr>");
        }
    },
